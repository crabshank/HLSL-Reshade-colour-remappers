...
c0XYZ=LinRGB2XYZ(c0.rgb,Mode);

c0hsv=rgb2hsv(c0.rgb);
c0hsv.z=1-c0hsv.z;

c1XYZ=LinRGB2XYZ(hsv2rgb(c0hsv),Mode);

c0xyY=XYZ2xyY(c0XYZ);
c1xyY=XYZ2xyY(c1XYZ);

return float4 (XYZ2LinRGB(xyY2XYZ(float3(c0xyY.xy,c1xyY.z)),Mode),c0.w);

}else{
	
c0XYZ=rgb2XYZ(c0.rgb,Mode);
c0hsv=rgb2hsv(c0.rgb);
c0hsv.z=1-c0hsv.z;

c1XYZ=rgb2XYZ(hsv2rgb(c0hsv),Mode);

c0xyY=XYZ2xyY(c0XYZ);
c1xyY=XYZ2xyY(c1XYZ);
return float4 (XYZ2rgb(xyY2XYZ(float3(c0xyY.xy,c1xyY.z)),Mode),c0.w);
