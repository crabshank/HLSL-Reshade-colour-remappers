...
float2x3 XYZog;
		...
		XYZog[0][0] = dot(v1, rgbLin);
		XYZog[0][1] = dot(v2, rgbLin);
		XYZog[0][2] = dot(v3, rgbLin);
    ...
    float3 rgbNew=rgbNewAvg;
		...
		XYZog[1][0] = dot(v1, rgbNew);
		XYZog[1][1] = dot(v2, rgbNew);
		XYZog[1][2] = dot(v3, rgbNew);
...
float3 WPChangeRGB(float3 color, float3 from, float3 to, int mode){
//lin == 1 only!
float2x3 XYZed=rgb2XYZ(color.rgb,mode,0, 1);
float3  XYZed2=float3( XYZed[0][0],XYZed[0][1],XYZed[0][2]);
return XYZ2rgb(WPconv(XYZed2,from,to),mode,1);
...
[branch]if(lnr==0){
c0.rgb=rgb2LinRGB(color.rgb,mde);
color=c0;
}

[branch]if (bdt==1){

float2x3 WPg= rgb2XYZ(float3(p3.xyz*rcptwoFiveFive),mde, 1,1);

	float3 WPgf= float3( WPg[0][0],WPg[0][1],WPg[0][2]);
	float3 WPgt= float3( WPg[1][0],WPg[1][1],WPg[1][2]);
...
[branch]if (dst==1){
float2x3 XYZed01=rgb2XYZ(color.rgb,mde,0,1);
float3 XYZed02=float3( XYZed01[0][0],XYZed01[0][1],XYZed01[0][2]);
color.rgb=XYZ2rgb(WPconv(WPconv(XYZed02,from,to),to,xy2XYZ(float2(p5.xy))),mde,1);
}else{
