...
float4 p2 :  register(c2); //avoid_grey, avoid_light, Debug, debug_val
float3 p3 :  register(c3); //Linear, mode, debug_type
float3 p4 :  register(c4); //red/green/blueDeltaAmnt
float3 p5 :  register(c5); //sat/rgb/YDeltaAmnt
float4 p6 : register(c6); //Red, Orange__Brown, Yellow, Chartreuse_Lime
float4 p7 : register(c7); //Green, Spring_green, Cyan, Azure__Sky_blue
float4 p8 : register(c8); // Blue, Violet__Purple, Magenta__Pink, Reddish_pink
...
#define rgbDeltaAmnt p5.y //avoid_grey and colour include settings have no effect on this setting
#define Y_DeltaAmnt p5.z //avoid_grey and colour include settings have no effect on this setting

#define MODE round(p3.y) // 0 - sRGB | 1 - Rec 601 NTSC | 2 - Rec. 601 PAL | 3 - Rec. 709 | 4 - Rec.2020 | 5 - DCI-P3 | 6 - Display P3 | 7 - Orginal NTSC (47 CFR ยง 73.682 - TV transmission standards) | 8 - Rec. 601 D93 | 9 - Rec. 709 D93 | 10 - DCI-P3 (D60/ACES)) | 11 -  Orginal NTSC D65

#define Linear round(p3.x) // 0-1 Take linear RGB as input and output linear RGB

#define avoid_grey round(p2.x)

#define avoid_light round(p2.y)

//Apply redDeltaAmnt/greenDeltaAmnt/blueDeltaAmnt to these colours
#define Red round(p6.x)
#define Orange__Brown round(p6.y)
#define Yellow round(p6.z)
#define Chartreuse_Lime round(p6.w)
#define Green round(p7.x)
#define Spring_green round(p7.y)
#define Cyan round(p7.z)
#define Azure__Sky_blue round(p7.w)
#define Blue round(p8.x)
#define Violet__Purple round(p8.y)
#define Magenta__Pink round(p8.z)
#define Reddish_pink round(p8.w)
...
float4 c1=change(c0,c0_hsv,Mode,linr);
	...
	int dbg=DEBUG;
	float dbg_v=DEBUG_val;
	int dbg_t=DEBUG_type;
