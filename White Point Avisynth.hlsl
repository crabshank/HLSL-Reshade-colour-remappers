...
float2x3 XYZed=rgb2XYZ(color.rgb,mode,0, lin);
float3  XYZed2=float3( XYZed[0][0],XYZed[0][1],XYZed[0][2]);
return XYZ2rgb(WPconv(XYZed2,from,to),mode,lin);
...
float2x3 WPg= rgb2XYZ(float3(p3.xyz*rcptwoFiveFive),mde, 1,lnr);
...
[branch]if (dst==1){
float2x3 XYZed01=rgb2XYZ(color.rgb,mde,0,lnr);
float3 XYZed02=float3( XYZed01[0][0],XYZed01[0][1],XYZed01[0][2]);
color.rgb=XYZ2rgb(WPconv(WPconv(XYZed02,from,to),to,xy2XYZ(float2(p5.xy))),mde,lnr);
}else{
...
float hue_dbg=120;
	...
	color.rgb=hsv2rgb(float3(hue_dbg/360.0,1,1-p4.w));
