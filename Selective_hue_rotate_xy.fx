...
Y_og =rgb2XYZ(c0.rgb,Mode).y;
}else{
Y_og =LinRGB2XYZ(c0.rgb,Mode).y;
}
...
[branch]if(Linear==false){
c1.rgb=XYZ2rgb(xyY2XYZ(float3(XYZ2xyY(rgb2XYZ(hsv2rgb(float3(new_hue,c0_hsv.yz)),Mode)).xy,Y_og)),Mode);
}else{
c1.rgb=XYZ2LinRGB(xyY2XYZ(float3(XYZ2xyY(LinRGB2XYZ(hsv2rgb(float3(new_hue,c0_hsv.yz)),Mode)).xy,Y_og)),Mode);
}
