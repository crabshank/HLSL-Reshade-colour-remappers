...
float4 White_Point_point_direct_Pass2D(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{

float4 c0=tex2D(ReShade::BackBuffer, texcoord); //current pixel
...
c1.rgb=((texcoord.x>=0.556 && texcoord.x<=0.616) && (texcoord.y<=0.023))?p0.rgb:c1.rgb;
return c1;


}

technique White_Point_point_direct
{
	pass
	{
		VertexShader = PostProcessVS;
		PixelShader = White_Point_point_direct_Pass2D;
	}
}
